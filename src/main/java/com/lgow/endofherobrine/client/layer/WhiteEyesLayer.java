...
public class WhiteEyesLayer<T extends PathfinderMob, M extends EntityModel<T>> extends RenderLayer<T, M> {
    ...
    public void render(PoseStack poseStack, MultiBufferSource source, int lightIn, T entity, float swing,
                       float swingAmount, float parTicks, float age, float headYaw, float headPitch) {
        float f = 1.0F;
        VertexConsumer vertex = source.getBuffer(getRenderType(entity));
        if (shouldRender(entity)) { this.getParentModel().renderToBuffer(poseStack, vertex, lightIn, OverlayTexture.NO_OVERLAY, f, f, f, f);}
    }
}
