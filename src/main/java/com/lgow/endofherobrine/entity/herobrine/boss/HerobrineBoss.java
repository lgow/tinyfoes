...
public class HerobrineBoss extends AbstractHerobrine implements RangedAttackMob {
    ...
    private static final EntityDataAccessor<Integer> DATA_ID_INV = SynchedEntityData.defineId(HerobrineBoss.class, EntityDataSerializers.INT);
    private static final EntityDataAccessor<Boolean> IS_ENRAGED = SynchedEntityData.defineId(HerobrineBoss.class, EntityDataSerializers.BOOLEAN);
    ...
    public void addAdditionalSaveData(CompoundTag pCompound) {
        ...
        pCompound.putBoolean("Enraged", this.getEnraged());
    }
    ...
    public boolean hurt(DamageSource pSource, float pAmount) {
        if (this.isInvulnerableTo(pSource)) {
            return false;
        } else if (pSource != DamageSource.DROWN && !(pSource.getEntity() instanceof HerobrineBoss)) {
    ...
    protected void dropCustomDeathLoot(DamageSource pSource, int pLooting, boolean pRecentlyHit) {
        ...
        if (this.getEnraged() && itementity != null) {
    ...
    public static AttributeSupplier.Builder createAttributes() {
        return Monster.createMonsterAttributes().add(Attributes.MAX_HEALTH, 20.0D).add(Attributes.MOVEMENT_SPEED, (double)0.6F).add(Attributes.FLYING_SPEED, (double)0.6F).add(Attributes.FOLLOW_RANGE, 40.0D).add(Attributes.ARMOR, 4.0D);
    }
    ...
    //todo follow player if he changes dimensions
