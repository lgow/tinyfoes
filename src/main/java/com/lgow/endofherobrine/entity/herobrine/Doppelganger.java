...
////todo player quits game doppleganger disappears
    ...
    public AbstractClientPlayer getClientPlayer() { return (AbstractClientPlayer) getPlayerByUUID();}

    public ServerPlayer getServerPlayer(){ return (ServerPlayer) getPlayerByUUID();}
    ...
    public HumanoidArm getMainArm() {
        if(getPlayerByUUID() != null){
            return this.getPlayerByUUID().getMainArm();
        } else {
    ...
    protected SoundEvent getHurtSound(DamageSource pDamageSource) {
		DamageSources source = this.damageSources();
        if (pDamageSource == source.onFire()) {
            return SoundEvents.PLAYER_HURT_ON_FIRE;
        } else if (pDamageSource == source.fall()) {
            return SoundEvents.PLAYER_HURT_DROWN;
        } else if (pDamageSource == source.sweetBerryBush()) {
            return SoundEvents.PLAYER_HURT_SWEET_BERRY_BUSH;
        } else {
            return pDamageSource == source.freeze() ? SoundEvents.PLAYER_HURT_FREEZE : SoundEvents.PLAYER_HURT;
        }
    }
    ...
    public boolean doHurtTarget(Entity target) {
        if(target instanceof ServerPlayer serverPlayer && !hasPlayerUUID()) {
            ...
            this.setPlayerUUID(target.getUUID());
...
//              this.removeEffect(MobEffects.INVISIBILITY);
    ...
    @Override
    public void tick() {
        Player player = this.getPlayerByUUID();
