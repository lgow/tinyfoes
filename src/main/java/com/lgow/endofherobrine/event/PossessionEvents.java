...
public class PossessionEvents {
	...
	public void onEntitySpawn(MobSpawnEvent event) {
		...
		if (ModConfigs.MOB_POSSESSION.get() && event.getLevel() instanceof ServerLevel server
				&& mob.tickCount == 1 && probability(server, 0.6F)) {
	...
	public void onMobHurt(LivingHurtEvent event) {
		if (ModConfigs.MOB_POSSESSION.get() && event.getEntity() instanceof Mob mob
				&& mob.level() instanceof ServerLevel server && event.getAmount() < mob.getHealth()) {
