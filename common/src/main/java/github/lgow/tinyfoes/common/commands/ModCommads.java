package github.lgow.tinyfoes.common.commands;
...
import github.lgow.tinyfoes.common.entity.BabyfiableEntity;
...
public class ModCommads {
	private static final List<ConfigValue<?>> serverList = List.of(TinyFoesConfigs.BABY_MAX_HEALTH_MODIFIER, TinyFoesConfigs.BABY_SPEED_MODIFIER,
			TinyFoesConfigs.SPAWN_AS_BABY_ODDS, TinyFoesConfigs.WITCH_GOAL);
	private static final List<ConfigValue<?>> clientList = List.of(
			TinyFoesConfigs.OVERSIZED_ITEMS, TinyFoesConfigs.VILLAGER_HEAD_FIX);
		...
		pDispatcher.register(Commands.literal("tinyfoes").requires((r) -> r.hasPermission(3))//
				.then(Commands.literal("server")//
						.then(Commands.literal("witchBabyficationGoal")
								.then(Commands.argument("value", BoolArgumentType.bool())//
										.executes((r) -> setConfigBoolean(r.getSource(), TinyFoesConfigs.WITCH_GOAL,
												BoolArgumentType.getBool(r, "value")))))//
						.then(Commands.literal("spawnAsBabyOdds")//
								.then(Commands.argument("value", DoubleArgumentType.doubleArg(0))//
										.executes((r) -> setConfigDouble(r.getSource(), TinyFoesConfigs.SPAWN_AS_BABY_ODDS,
												DoubleArgumentType.getDouble(r, "value")))))//
						.then(Commands.literal("babySpeedModifier")
								.then(Commands.argument("value", DoubleArgumentType.doubleArg(0))//
										.executes((r) -> setConfigDouble(r.getSource(), TinyFoesConfigs.BABY_SPEED_MODIFIER,
												DoubleArgumentType.getDouble(r, "value")))))
						.then(Commands.literal("babyHealthModifier")
								.then(Commands.argument("value", DoubleArgumentType.doubleArg(0))//
										.executes((r) -> setConfigDouble(r.getSource(), TinyFoesConfigs.BABY_MAX_HEALTH_MODIFIER,
												DoubleArgumentType.getDouble(r, "value")))))//
				...
				.then(Commands.literal("client")//
						.then(Commands.literal("villagerHeadFix")
								.then(Commands.argument("value", BoolArgumentType.bool())//
										.executes((r) -> setConfigBoolean(r.getSource(), TinyFoesConfigs.VILLAGER_HEAD_FIX,
												BoolArgumentType.getBool(r, "value")))))//
						.then(Commands.literal("oversizedItems")
								.then(Commands.argument("value", BoolArgumentType.bool())//
										.executes((r) -> setConfigBoolean(r.getSource(), TinyFoesConfigs.OVERSIZED_ITEMS,
												BoolArgumentType.getBool(r, "value")))))//
